...
sudo -- su postgres -c "createdb -O ${WERCKER_POSTGRESQL_USERNAME} werkerdb$TEST_ENV_NUMBER"
PGPASSWORD=$WERCKER_POSTGRESQL_PASSWORD psql -c 'CREATE EXTENSION postgis;' -U $WERCKER_POSTGRESQL_USERNAME -d werckerdb$TEST_ENV_NUMBER -h $WERCKER_POSTGRESQL_HOST -p $WERCKER_POSTGRESQL_PORT
PGPASSWORD=$WERCKER_POSTGRESQL_PASSWORD psql -c 'CREATE EXTENSION postgis_topology;' -U $WERCKER_POSTGRESQL_USERNAME -d werckerdb$TEST_ENV_NUMBER -h $WERCKER_POSTGRESQL_HOST -p $WERCKER_POSTGRESQL_PORT
